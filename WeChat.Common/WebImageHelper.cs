...
namespace WeChat.Common
{
    public class WebImageHelper
    {
        ...
        public WebImageHelper(byte[] content)
        {
        ...
        public WebImageHelper(string filePath)
            : this(new HttpContextWrapper(HttpContext.Current), _defaultReadAction, filePath)
        {
        }

        public WebImageHelper(Stream imageStream)
        {
        ...
        internal WebImageHelper(HttpContextBase httpContext, Func<string, byte[]> readAction, string filePath)
        {
            if (String.IsNullOrEmpty(filePath))
            {
                throw new ArgumentException("不能为空或null", "filePath");
            }

            _fileName = filePath;
            _content = readAction(filePath);
            _initialFormat = ValidateImageContent(_content, "filePath");
            _currentFormat = _initialFormat;
        }

        private WebImageHelper(WebImageHelper other)
        {
        ...
        public string ImageFormat
        {
            get
            {
                ...
                return Conversion.ToString(_currentFormat).ToLowerInvariant();
            }
        }

        public static WebImageHelper GetImageFromRequest(string postedFileName = null)
        {
            var request = new HttpRequestWrapper(HttpContext.Current.Request);
            return GetImageFromRequest(request, postedFileName);
        }

        internal static WebImageHelper GetImageFromRequest(HttpRequestBase request, string postedFileName = null)
        {
            ...
            //if (!ConversionHelper.TryFromStringToImageFormat(mimeType, out format))
            //{
            //    // Unsupported image format.
            //    return null;
            //}

            WebImageHelper webImage = new WebImageHelper(file.InputStream);
            webImage.FileName = file.FileName;
            return webImage;
        }

        public WebImageHelper Clone()
        {
            return new WebImageHelper(this);
        }
        ...
        public WebImageHelper Resize(int width, int height, bool preserveAspectRatio = true, bool preventEnlarge = false)
        {
            if (width <= 0)
            {
        ...
        public WebImageHelper Crop(int top = 0, int left = 0, int bottom = 0, int right = 0)
        {
            if (top < 0)
            {
                throw new ArgumentOutOfRangeException(
        ...
        public WebImageHelper RotateLeft()
        {
        ...
        public WebImageHelper RotateRight()
        {
        ...
        public WebImageHelper FlipVertical()
        {
        ...
        public WebImageHelper FlipHorizontal()
        {
        ...
        public WebImageHelper AddTextWatermark(
            string text,
            ...
            int padding = 5)
        {
            ...
            {
                throw new ArgumentException("水印文字不能为空或null", "text");
            }

            Color color;
            if (!Conversion.TryFromStringToColor(fontColor, out color))
            {
                throw new ArgumentException("不正确的颜色格式");
            }
            ...
            if (!Conversion.TryFromStringToEnum(fontStyle, out fontStyleEnum))
            {
                throw new ArgumentException("不正确的字体样式");
            }

            FontFamily fontFamilyClass;
            if (!Conversion.TryFromStringToFontFamily(fontFamily, out fontFamilyClass))
            {
                throw new ArgumentException("不正确的字体");
            }
        ...
        /// <returns>Modified WebImage instance with added watermark.</returns>
        public WebImageHelper AddImageWatermark(
            WebImageHelper watermarkImage,
            int width = 0,
            int height = 0,
            string horizontalAlign = "Right",
            string verticalAlign = "Bottom",
            ...
            int padding = 5)
        {
            ...
            {
                throw new ArgumentException("图片的宽度和高度不正确");
            }
            if ((opacity < 0) || (opacity > 100))
            {
                throw new ArgumentOutOfRangeException("opacity", "参数的大小必须在0-100之间");
            }
            ...
            if (padding < 0)
            {
                throw new ArgumentOutOfRangeException(
                    "padding",
                    String.Format(CultureInfo.InvariantCulture, "参数必须大于等于0"));
            }
        ...
        internal WebImageHelper AddImageWatermark(
            ...
            int padding)
        {
            return AddImageWatermark(new WebImageHelper(httpContext, readAction, watermarkImageFilePath), width, height, horizontalAlign, verticalAlign, opacity, padding);
        }

        [SuppressMessage("Microsoft.Globalization", "CA1308:NormalizeStringsToUppercase",
            Justification = "No security decision is made based on this string. It's customary to display MIME format in lowercase.")]
        public WebImageHelper Write(string requestedFormat = null)
        {
        ...
        public WebImageHelper Save(string filePath = null, string imageFormat = null, bool forceCorrectExtension = true)
        {
        ...
        internal WebImageHelper Save(HttpContextBase context, Action<string, byte[]> saveAction, string filePath, string imageFormat, bool forceWellKnownExtension)
        {
            filePath = filePath ?? FileName;
            if (String.IsNullOrEmpty(filePath))
            {
                throw new ArgumentNullException("filePath", "参数不能为空或null");
            }
            ...
            if (forceWellKnownExtension)
            {
                ...
                if (!Conversion.TryFromStringToImageFormat(extension, out saveImageFormat) || !saveImageFormat.Equals(requestedImageFormat))
                {
        ...
        private static ImageFormat ValidateImageContent(byte[] content, string paramName)
        {
            ...
            {
                throw new ArgumentException("不是有效的图像格式", paramName, exception);
            }
        }
        ...
        private static ImageFormat GetImageFormat(string format)
        {
            ...
            ImageFormat result;
            if (!Conversion.TryFromStringToImageFormat(format, out result))
            {
                throw new ArgumentException(String.Format(CultureInfo.InvariantCulture, "不正确的图像格式{0}", format), "format");
            }
        ...
        private static HorizontalAlign ParseHorizontalAlign(string alignment)
        {
            ...
            conversionOk = Conversion.TryFromStringToEnum(alignment, out horizontalAlign);
            if (!conversionOk || (horizontalAlign == HorizontalAlign.Justify) || (horizontalAlign == HorizontalAlign.NotSet))
            {
                throw new ArgumentException("不正确的水平对其方式");
            }
        ...
        private static VerticalAlign ParseVerticalAlign(string alignment)
        {
            ...
            conversionOk = Conversion.TryFromStringToEnum(alignment, out verticalAlign);
            if (!conversionOk || (verticalAlign == VerticalAlign.NotSet))
            {
                throw new ArgumentException("不正确的垂直对齐方式");
            }
        ...
        private class WatermarkImageTransformation : WatermarkTransformation
        {
            ...
            public WebImageHelper WatermarkImage { get; set; }
            public int Width { get; set; }
            public int Height { get; set; }
