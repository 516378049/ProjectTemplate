...
{
	/// <summary>
	/// This class is responsible for making HTTP requests and returning the response
	/// </summary>
	public static class HttpClientHelper
	{
        ...
        {
            HttpWebResponse response = HttpClientHelper.Get(strURL,header);
            string responseString = HttpClientHelper.GetStringFromStream(response.GetResponseStream(), response.ContentLength);
            return JsonConvert.DeserializeObject<T>(responseString);
        }

        public static T Get<T>(string strURL)
        {
            HttpWebResponse response = HttpClientHelper.Get(strURL);
            string responseString = HttpClientHelper.GetStringFromStream(response.GetResponseStream(), response.ContentLength);
            return JsonConvert.DeserializeObject<T>(responseString);
        }
        ...
        {
            HttpWebResponse response = HttpClientHelper.Post(strURL,JsonConvert.SerializeObject(strData));
            string responseString = HttpClientHelper.GetStringFromStream(response.GetResponseStream(), response.ContentLength);
            return JsonConvert.DeserializeObject<T>(responseString);
        }
        public static T Post<T,DATA>(string strURL,DATA strData,Dictionary<string,string> header)
        {
            HttpWebResponse response = HttpClientHelper.Post(strURL,header,Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(strData)));
            string responseString = HttpClientHelper.GetStringFromStream(response.GetResponseStream(), response.ContentLength);
            return JsonConvert.DeserializeObject<T>(responseString);
        }
        public static T Post<T>(string strURL, string strData)
        {
            HttpWebResponse response = HttpClientHelper.Post(strURL, strData);
            string responseString = HttpClientHelper.GetStringFromStream(response.GetResponseStream(), response.ContentLength);
            return JsonConvert.DeserializeObject<T>(responseString);
        }
