...
namespace OrderMeal.Controllers
{
    ...
    public class SellerAdminController : BaseApiController
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public ApiResult getSeller(int id=-1)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        public ApiResult getSellerList(int pageNum = 1, int pageSize = 10,string name="",int delflag=0, string orderBy = "UpdateTime", string sort = "DESC")
        {
            try
            {
                ...
                {
                    return CreateApiExpResult("商家状态参数错误！");
                }
                
                List<Expression<Func<sellers, bool>>> query = new List<Expression<Func<sellers, bool>>>();
                Func<IQueryable<sellers>, IOrderedQueryable<sellers>> order = X=>X.OrderByDescending(Y=>Y.UpdateTime);
                query.Add(X => X.DelFlag == delflag);
                if (!string.IsNullOrWhiteSpace(name))
                {
                    query.Add(X => X.name .Contains(name));
                }
                List<sellersEx> sellersExList = new List<sellersEx>();
                List<sellers> sellerList = Studio.Sellers.Get(query, order,"", pageNum, pageSize).ToList();
                sellerList.ForEach(seller =>
                {
                    List<supports> supports = Studio.Supports.Get(X => X.sellerId == seller.Id && X.DelFlag == 0).ToList();
                    sellersEx sellerEx = new sellersEx(seller);
                    sellerEx.supports = supports;
                    sellersExList.Add(sellerEx);
                });

                return CreateApiResult(sellersExList,sellersExList.Count);
            }
            catch (Exception ex)
            {
                return ErrorHandle(ex);
        ...
        /// <returns></returns>
        [HttpPost]
        public ApiResult saveSeller(sellers seller)
        {
            sellers sellerSave = new sellers();
            //前端参数
            string Params = GetRequestStreamData();
            //获取修改的字段
            Dictionary<string, object> fileds = JsonHelper.ToObject<Dictionary<string, object>>(Params);
            //获取数据模型
            EntityHelper.EntityCopyFromDic(fileds, sellerSave);
            if (sellerSave.Id > 0)
            {
                Studio.Sellers.UpdateEntityFields(sellerSave, fileds.Keys.ToList());//修改操作
            }
            else
            {
                Studio.Sellers.Insert(sellerSave);//修改操作
            }
