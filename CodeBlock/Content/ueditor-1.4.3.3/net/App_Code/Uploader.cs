...
public class Uploader
{
  ...
  * @return Hashtable
  */
    public Hashtable upFile( string pathbase, string[] filetype, int size)
    {
        pathbase = pathbase + DateTime.Now.ToString("yyyy-MM-dd") + "/";
        uploadpath = HttpContext.Current.Server.MapPath(pathbase);//获取文件上传路径

        try
        {
            uploadFile = HttpContext.Current.Request.Files[0];
